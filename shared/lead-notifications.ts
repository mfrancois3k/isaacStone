...
export async function notifyLead(lead: LeadRecord): Promise<LeadNotificationResult> {
  ...
  const customerText = `Hi ${customerName} — Isaac Stone and Tile received your request. Wamy sent it to our team for review. ${estimateRange(lead).ready ? `Your preliminary range is ${money(estimateRange(lead).low)}–${money(estimateRange(lead).high)}; final pricing follows site review and verified measurements. ` : ''}Your requested visit still needs team confirmation. For immediate help, call (631) 530-5883.`;
